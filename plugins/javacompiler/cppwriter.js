let fs = require("fs");
let platform = require("process").argv[3];
    ...
    function writeTypes( t ){
        ...
        if( t.cppType == "enum class" ){
            ...
            t.constantList.forEach( c => {
                out += `${indent}static const ue_${t.name} ${c.name};\n`;
            });
            pop();
            out += `${indent}};\n\n`;
        }else{
...
`;
            if( t.fields.length ){
                ...
                push();
                out += `${indent}if(__generation__ == __next_generation__) return;\n`;
                if( t.extends ){

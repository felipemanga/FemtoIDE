...
function onFemtoReady(){
    const path = require("path");
    var buffer = APP.findFile(path.join(DATA.projectPath, "game.html"));
    try{
        let gameDataStr = APP.readBufferSync( buffer );
        gameDataStr = exporter.importGame( gameDataStr );
        document.getElementById("game_data").value = gameDataStr;
        on_game_data_change();
        paintExplorer.Refresh(drawing.type);
    }catch(ex){}

    ExporterUtils.DownloadFile = function(filename, text){
        buffer.data = text;
        buffer.modified = true;
        APP.writeBuffer(buffer);
        APP.log("Saved game.html");
    };
}

function onFemtoCompile(){
    exportGame();
}
